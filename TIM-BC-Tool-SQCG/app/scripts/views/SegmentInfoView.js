  ...
  'controllers/SegmentValidateCTRL'
    ...
    var SegmentInfo = Backbone.View.extend({
      ...
      events : {
        ...
        // VALIDATIONS FOR THE NEW TIM-TF USER INPUTS
        'change #proportion': 'tfValidation',
        'change #implementation': 'tfValidation',
        'change #averageImprovement': 'tfValidation'
      },

      tfValidation : function( evnt ) {
        var _this = this;
        ...
        if( validInput ) {
          switch( item.id ) {
            case 'averageImprovement':
              if( item.value() <= 240 && item.value() > 0 ) {
                _this.currentSegment.set( item.id, Number( item.value() ));
              } else {
                item.element.focus();
                alert('Error: Improved Duration should be a number between 10 - 240.');
      ...
      updateNumberOfLanesByDirection: function( evnt ) {
        ...
        if( isNaN( value ) || value < 2 || value > 6 ) {
          ...
          var resp = window.confirm('Changing the number of lanes will reset all values in the Incident Information panel to zero. It will also reset any incident duration savings eneterd by lane blocakge. Continue?');
          if( resp ) {
