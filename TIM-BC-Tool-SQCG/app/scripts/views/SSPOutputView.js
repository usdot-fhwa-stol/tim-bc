...
define([
  ...
  var SSPOutputView = Backbone.View.extend({
    ...
    // @desc Get secondary incidents without TF savings
    // @param withoutTFsavings is the computed peak savings with the shortened duration added
    // @param withTFsavings is the user input values a.k.a. the results when TIM TF is applied
    getSecondaryIncidents: function( withoutTFsavings, withTFsavings ) {
      var totalSecondaryIncidentsWithoutTF = 0;
      var totalTravelDelayWithoutTF = 0;
      var totalTravelDelayWithTF = 0;
      // traverse through all incidents by weather percentage object to get the total travel
      // delays for both with(User Values) and without(Computed Values) TIM TF savings
      for( var x = 0, len = withoutTFsavings.length; x < len; x++ ) {
        var incidentsByWeatherWithoutTF = withoutTFsavings[x].incidentsByWeather;
        var incidentsByWeatherWithTF = withTFsavings[x].incidentsByWeather;

        for( var i = 0, len2 = incidentsByWeatherWithoutTF.length; i < len2; i++ ) {
          var blockage = incidentsByWeatherWithoutTF[i].blockage;
          totalTravelDelayWithoutTF += incidentsByWeatherWithoutTF[i].totalTravelDelayCarAndTruck;
          totalTravelDelayWithTF += incidentsByWeatherWithTF[i].totalTravelDelayCarAndTruck;
        }
      }
      ...
      for( var x = 0, len = withoutTFsavings.length; x < len; x++ ) {
        ...
        for( var i = 0, len2 = incidentsByWeatherWithoutTF.length; i < len2; i++ ) {
          var secondaryIncidentsWithTF = incidentsByWeatherWithTF[i].secondaryIncidents;
          // calculate for the without TIM TF savings secondary incidents
          // based on SSP tool's version:
          // secondaryIncidentsWithoutSSP =  secondaryIncidents * allTDWithoutSSP  / allTDWithSSP
          var secondaryIncidentsWithoutTF = secondaryIncidentsWithTF * totalTravelDelayWithoutTF / totalTravelDelayWithTF;
          secondaryIncidentsWithTF = ( isNaN(secondaryIncidentsWithTF) ) ? 0 : secondaryIncidentsWithTF;
          // sum all secondary incidents withouts TIM TF savings
          totalSecondaryIncidentsWithoutTF += secondaryIncidentsWithoutTF;
        }

      }

      return totalSecondaryIncidentsWithoutTF;
    },
