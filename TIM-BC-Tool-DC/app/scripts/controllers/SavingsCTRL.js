...
define([
	...
	var SavingsCTRL = function() {
		...
		this.getBlockageNum = {
			...
			//Deleted by FZ for DC-BC-TOOL 07/10/2015
			//'improvedOneLaneBlockage': function() {
			//	return 1;
			//},
			////Deleted end for DC-BC-TOOL 07/10/2015
		...
		this.getSpeedAVG = function( speed, segmentName ) {
			...
			if( speed < 60 || speed > 120 ) {
				//Deleted by Fang Zhou for deleting speed range error message on 09/10/2015
				//GlobalEvent.trigger('errorSpeed', { name: segmentName });
				//Deleted end for deleting speed range error message on 09/10/2015
			}
      ...
      var shortenedDuration = Number( segment.get('shortenedDuration') );
			...
			// get the compliance rate to calculate againts the proportion
			var complianceRate = Number( segment.get('complianceRate') ) / 100;
			complianceRate = proportion * complianceRate;
      ...
      var improvedDuration = shortenedDuration;
