...
define([
  ...
  var ProjectDetails = Backbone.View.extend({
    ...
    initialize : function( options ) {
      ...
      GlobalEvent.on('submit-calculation project-file-uploaded project:details', function() {
        self.render();
      });
    ...
    updateSegments : function( evt ) {
      ...
      if( evt != undefined ) {
        ...
        input.value = Number( $('#number_of_segments').val() );
      }
      ...
      if( this.validateInput(input, 'numOfSegments') ) {
        if( input.value > 0 && input.value <= 30 ) {
          ...
          alert('Error: Allowed number of Segments is 1 - 30 only!');
          $(evt.currentTarget).focus().val( this.model.get('numOfSegments') );
        }
      }
