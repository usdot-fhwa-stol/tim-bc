  ...
  'controllers/SegmentValidateCTRL'
    ...
    var SegmentInfo = Backbone.View.extend({
      ...
      events : {
        ...
        // VALIDATIONS FOR THE NEW PTSA USER INPUTS
        'change #shortenedDuration': 'ptsaValidation',
        'change #proportion': 'ptsaValidation',
        'change #complianceRate': 'ptsaValidation',
      },

      ptsaValidation : function( evnt ) {
        var _this = this;
          ...
          switch( item.id ) {
            ...
            case 'complianceRate':
              if( item.value() <= 100 && item.value() >= 0 ) {
                ...
                item.element.focus();
                alert('Error: Compliance Rate should be a number between 0 - 100.');
                item.value( item.modelValue );
              }
              break;
          }
