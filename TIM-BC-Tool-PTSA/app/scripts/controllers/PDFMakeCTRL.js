...
define(['underscore'], function() {
	...
	var sspPDF = {
		...
		getValues: function( project ) {
				...
				if( segment.get('checked') != false && segment.get('valid') != false ) {
	   			...
	   			// Removed by Robert Roth to avoid generating an extra blank page after segments pages
					...
					var SEGMENT_PARTIAL = [
		   			...
		   			//Added by Fang Zhou for adding Monetary conversion rates table on 09/11/2015
            {
            ...
            //Added end for adding Monetary conversion rates table on 09/11/2015
		   			{
		   				text: '',
							pageBreak: "after"
		   			}
		...
		generate : function( project ) {
		    ...
		    content: [
		    	// First Page
	   			{
	   				text: 'PTSA B/C Tool Output\n' + PROJECT_NAME + '\n' + _this.getDate(),
	   				alignment: 'right'
	   			},

	        {
	            text: 'Pre-established Towing Service Agreements Benefit/Cost Estimations Tool Output',
	            fontSize: 24,
	            alignment: 'justified',
	            margin: [50, 200, 50, 0],
	            color: '#5379A8',
...

	        {
	            ...
	            margin: [50, 10, 0, 0],
	   			...
	   			{
	   				text:'Several important benefits that can be derived from an PTSA program are accounted for in the B/C ratio developed by the PTSA-BC Tool. The tool was purposefully developed to provide conservative, defensible estimates. This section outlines numerous additional benefits that have not been included in the B/C ratio, but that would increase its value if considered.'
	   			},
	   			...
	   			{
	   				text: 'The B/C ratio calculation only considers the monetary value of property damage that might be prevented in the event of a secondary incident due to the existence of an PTSA program. The B/C ratio would be expected to rise very significantly if the value of even one fatal or near-fatal incident were included. Additionally, the savings derived from avoiding congestion resulting from secondary incidents due to the PTSA program have not been included in estimating the savings from secondary incidents.  '
	   			},
			...
			return pdfMake.createPdf(dd).download('PTSA-Report.pdf');
		}//--
	}
