...
function ProjectTemplates() {
  ...
  */
  ...
  this.FourLaneSavings = [
	...
	*/
	this.ListTemplate = [
      ...
      '</div>',
      '<label><input type="checkbox" tabindex="1" data-index="{{ index }}" class="segment-checkbox" {{ segmentChecked }}> <span class="segment-name">{{ segmentName }}</span> </label>',
    '</div>'
	].join('\n');
  ...
  this.WeatherTemplate = [ 
    '<tr data-index="{{ id }}">',
      '<td>',
        '<select name="Weather Information" id="weather_{{ id }}" class="form-control weather" value="{{ type }}"  tabindex="1">',
