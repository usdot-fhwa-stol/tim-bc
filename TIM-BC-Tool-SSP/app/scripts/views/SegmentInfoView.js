...
define([
  ...
  'controllers/SegmentValidateCTRL'
    ...
    var SegmentInfo = Backbone.View.extend({
      ...
      initialize : function() {
        // cache controller to keep scope of 'this'
        var _self = this;
        //initialize weather information row count
        _self.weatherInfoRows = 1;
        // create an array to hold weather rows
        _self.weatherRows = [];
        // get out current segment if available
        if(_self.model.get('segments') != undefined ) {
          _self.currentSegment = _self.model.get('segments')[0];
        } else {
          _self.currentSegment = null;
        }
        // instantiate our segment validation controller
        _self.validateCTRL = new SegmentValidateCTRL();
        // ensure segment info view renders on duplication or
        // removal of segments using segment manager view
        GlobalEvent.on('segments:changed', _self.render, _self);
      },
      ...
      render : function( id, segment ) {
        ...
        if( _this.model.get('projectName') !== '' ) {
          // check if annual fixed cost is entered
          if( _this.model.get('totalProgramCost') > 0 ) {
