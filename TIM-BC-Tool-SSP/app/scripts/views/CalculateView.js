  ...
  'utilities/template'
    ...
    var CalculateView = Backbone.View.extend({
      ...
      initialize : function() {
        ...
        //--
        // Listen when Project State is changed and recalculate rows
        this.listenTo( this.model, 'change:projectState', this.updateValues );
        //--
      ...
      // Create an object when called either by keyup
      // within any input box in a table row or called
      // by a function. Object represents values entered
      // by users.
      // @params evnt - Event options when tiggered by keyup.
      //                Enter false when calling as a function.
      //         row  - A table row element.
      getRowValues: function( evnt, row ) {
        if( evnt ) this.clearEvent( evnt );
        if( row ) {
          row = $(row);
        } else {
      ...
      // This updates values when any change was made to the
      // Project Model, specifically Project State:
      updateValues: function() {
        var _this = this;
        var tableRows = $('.table-row');
        _.each( tableRows, function( row ) {
          var rowValues = _this.getRowValues( false, row );
          _this.calculateAll( rowValues );
        });
        if( _this.calculations.length > 0 ) {
          _this.submitCalculation();
        }
      },
      ...
      submitCalculation: function( evnt ) {
        ...
        if( isNaN( this.calculationTotal ) ) {
          ...
          GlobalEvent.trigger('overlay-close-triggerless');
          this.close();
        }
        return this;
      },
