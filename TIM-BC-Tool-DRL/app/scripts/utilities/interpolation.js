	...
	var Interpolate = function() {
			...
			if( typeof type === 'string' && typeof value === 'number' ) {
				...
				if( type === 'duration' ) {
					if( value >= 1 && value <= 14400 ) {
						...
						GlobalEvent.trigger('interpolation:error', { type: 'duration', err: 'Error: Duration should be between 1 - 14400.' });
						return 0;
					}
				} else if( type === 'speed' ) {
					if( value >= 60 && value <= 120 ) {
						...
						// return 'Error: Speed should be between 60 - 120.'
					}
				} else if( type === 'volume' ) {
					if( value >= 500 && value <= 2200 ) {
						...
						// return 'Error: Volume should be between 500 - 2200.';
					}
				} else {
					return 'Error: Invalid "type" or undefined.';
				}
			} else if( typeof type !== 'string' ) {
				return 'Error: Please enter string "type" first, e.g. "duration", and then enter a number "value".';
			}
		};
		...
		this.getUpper = function( type ) {
			...
			if( typeof value === 'number' ) {
				...
				return 'Error: ' + type + ' given is not a number.';
			}
		};
		...
		this.calculateWeights = function() {
			...
			if( typeof duration.value ==='number' && typeof speed.value ==='number' && typeof volume.value ==='number' ) {
				...
				return 'Error: Undefined or invalid attribute.'
			}
