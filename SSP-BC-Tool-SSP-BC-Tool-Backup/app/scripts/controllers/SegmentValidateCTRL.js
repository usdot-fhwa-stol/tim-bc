...
define([
	'underscore',
      ...
      this.validateRoadwayGeometry = function( evnt ) {
        ...
        var inputs = ['#segment_length', '#num_of_traffic_lanes'];
        var flag = true;
      ...
      this.validateProgramInformation = function ( segment ) {
        ...
        var iDurationValid = this.validateIncidentDurations( segment );
        console.log(opTimeValid + ' --:-- ' + iDurationValid );
        if( opTimeValid && iDurationValid ) {
          $('#ssp_program_information')
          ...
          if( value <= 0 ) {
            ...
            _.each( arrDurations, function( duration ) {
              ...
              if( ! duration.closest('.peak-incident').hasClass('hidden') ) {
                ...
                if( duration.val() > 0 && value > duration.val() ) {
                  hasErrors.push( value );
                }
              }
            });
            ...
            _.each( arrDurations, function( duration ) {
              ...
              if( ! $(duration).closest('.peak-incident').hasClass('hidden')) {
                ...
                if( savBlock == durBlock ) {
                  if( ! parseInt($(saving).val()) > 0 || parseInt($(saving).val()) > parseInt($(duration).val()) ) {
                    hasErrors.push($(saving).val());
                  }
                }
              }
            });
      ...
      this.validateTrafficVolume = function( evnt ) {
      ...
      ***********************************/
      ...
      this.validationTotalOfManagedIncidents = function( project ) {
        ...
        var valid = Number(total) === Number(project.get('numOfIncidentsOnProgramRoadway'));
