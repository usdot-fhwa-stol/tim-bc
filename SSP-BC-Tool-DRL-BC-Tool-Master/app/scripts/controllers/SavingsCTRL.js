...
define([
	...
	var SavingsCTRL = function() {
        ...
        var shortenedDuration = Number( segment.get('shortenedDuration') );
				...
				// if DRL is used get the shortened duration, else keep it zero.	
				//shortenedDuration = ( driverRemovalLaw == true ) ? shortenedDuration : 0;
				...
				objPeak = {
					...
					shortenedDuration: shortenedDuration
				};
				...
				//if( driverRemovalLaw == true) {
					...
					switch( segment.get('numberOfTrafficLanesByDirection') ) {
						...
						default:
							// Calculate Driver Removal Law Compliance.
							//_this.calculateDriverRemoval( segment );
							var oneLaneIncidents = Number( segment.get( strPeak + 'OneLaneBlockageManIndt') );
							var oneLaneDuration =  Number( segment.get( strPeak + 'OneLaneBlockageAVGID') );
			        ...
			        var compliantDrivers = complianceRate * oneLaneIncidents;
			        var nonCompliantDrivers = ( 1 - complianceRate ) * oneLaneIncidents;
			        // get improved duration
			        var improvedDuration = oneLaneDuration - shortenedDuration;
							// oneLaneDuration = oneLaneDuration - shortenedDuration;
							var improvedOneLaneDuration = ( driverRemovalLaw == true ) ? oneLaneDuration + improvedDuration : oneLaneDuration;
							var increasedShoulderDuration = oneLaneDuration - shortenedDuration;
							if ( driverRemovalLaw == true ) increasedShoulderDuration *= 2;

							blockages.push({
								name: 'oneLaneBlockage',							
								savings: Number( avgDurationSavings ),
								avgIncidentDuration:  _this.incidentDurationAVG( oneLaneDuration ),
								...
								numManagedIncidents: nonCompliantDrivers
							});
							blockages.push({
								name: 'improvedOneLaneBlockage',							
								savings: 0,
								avgIncidentDuration:  _this.incidentDurationAVG( improvedOneLaneDuration ),
								avgIncidentDurationWithSavings: _this.incidentDurationAVG( segment.get( strPeak + 'ImprovedOneLaneBlockageAVGID') ),
								numManagedIncidents: compliantDrivers
							});
							blockages.push({
							...
							blockages.push({
								...
								savings: 0,
								avgIncidentDuration: _this.incidentDurationAVG( increasedShoulderDuration ),
