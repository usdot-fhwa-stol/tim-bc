...
define([
	...
	var SavingsCTRL = function() {
		...
		this.getBlockageNum = {
			...
			'increasedShoulderBlockage': function() {
				return 0;
			},
		...
		this.getCompliance = function( segment, strPeak, lane, driverRemovalLaw ) {
      // For Driver Removal Laws
      var proportion = Number( segment.get('proportion') ) / 100;
      var complianceBefore = Number( segment.get('complianceBefore') );
      var complianceAfter = Number( segment.get('complianceAfter') );
      var shortenedDuration = Number( segment.get('shortenedDuration') );
			...
			var complianceRate = ( complianceAfter - complianceBefore ) * proportion / 100;
      var compliantDrivers = complianceRate * laneIncidents;
      var nonCompliantDrivers = ( 1 - complianceRate ) * laneIncidents;
      // get improved duration
      var improvedDuration = laneDuration - shortenedDuration;
      improvedDuration = ( improvedDuration < 0 ) ? 0 : improvedDuration;
			// laneDuration = laneDuration - shortenedDuration;
			var improvedLaneDuration = ( driverRemovalLaw == true ) ? laneDuration + improvedDuration : laneDuration;
			var increasedShoulderDuration = laneDuration - shortenedDuration;
			increasedShoulderDuration = ( increasedShoulderDuration < 0 ) ? 0 : increasedShoulderDuration;
			if ( driverRemovalLaw == true ) increasedShoulderDuration *= 2;
		...
		this.buildPeak = function( segment, strPeak, driverRemovalLaw ) {
			var _this = this;
			var objPeak = {};
			...
			if( segment.get( strPeak + 'Visibility' ) === 'visible' ) {
				...
				switch( segment.get('numberOfTrafficLanesByDirection') ) {
					case 5:
					case 6:
						var arlFourLane = _this.getCompliance( segment, strPeak, 'FourLane', driverRemovalLaw );
						blockages.push({
							name: 'fourLaneBlockage',							
							savings: Number( avgDurationSavings ),
							avgIncidentDuration:  _this.incidentDurationAVG( arlFourLane.laneDuration ),
							...
							numManagedIncidents: arlFourLane.nonCompliantDrivers
						});
						blockages.push({
							name: 'improvedFourLaneBlockage',							
							savings: Number( avgDurationSavings ),
							avgIncidentDuration:  _this.incidentDurationAVG( arlFourLane.improvedLaneDuration ),
							...
							numManagedIncidents: arlFourLane.compliantDrivers
						});
						blockages.push({
							name: 'increasedShoulderBlockage',
							laneSaved: 'Four Lane Blockage',
							savings: 0,
							avgIncidentDuration: _this.incidentDurationAVG( arlFourLane.increasedShoulderDuration ),
							avgIncidentDurationWithSavings: _this.incidentDurationAVG( segment.get( strPeak + 'IncreasedShoulderBlockageAVGID') ),
							numManagedIncidents: arlFourLane.compliantDrivers
						});
					case 4:
						var arlThreeLane = _this.getCompliance( segment, strPeak, 'ThreeLane', driverRemovalLaw );
						blockages.push({
							name: 'threeLaneBlockage',							
							savings: Number( avgDurationSavings ),
							avgIncidentDuration:  _this.incidentDurationAVG( arlThreeLane.laneDuration ),
							...
							numManagedIncidents: arlThreeLane.nonCompliantDrivers
						});
						blockages.push({
							name: 'improvedThreeLaneBlockage',							
							savings: Number( avgDurationSavings ),
							avgIncidentDuration:  _this.incidentDurationAVG( arlThreeLane.improvedLaneDuration ),
							...
							numManagedIncidents: arlThreeLane.compliantDrivers
						});
						blockages.push({
							name: 'increasedShoulderBlockage',
							laneSaved: 'Three Lane Blockage',
							savings: 0,
							avgIncidentDuration: _this.incidentDurationAVG( arlThreeLane.increasedShoulderDuration ),
							avgIncidentDurationWithSavings: _this.incidentDurationAVG( segment.get( strPeak + 'IncreasedShoulderBlockageAVGID') ),
							numManagedIncidents: arlThreeLane.compliantDrivers
						});
					case 3:
						var arlTwoLane = _this.getCompliance( segment, strPeak, 'TwoLane', driverRemovalLaw );
						blockages.push({
							name: 'twoLaneBlockage',							
							savings: Number( avgDurationSavings ),
							avgIncidentDuration:  _this.incidentDurationAVG( arlTwoLane.laneDuration ),
							...
							numManagedIncidents: arlTwoLane.nonCompliantDrivers
						});
						blockages.push({
							name: 'improvedTwoLaneBlockage',							
							savings: Number( avgDurationSavings ),
							avgIncidentDuration:  _this.incidentDurationAVG( arlTwoLane.improvedLaneDuration ),
							...
							numManagedIncidents: arlTwoLane.compliantDrivers
						});
						blockages.push({
							name: 'increasedShoulderBlockage',
							laneSaved: 'Two Lane Blockage',
							savings: 0,
							avgIncidentDuration: _this.incidentDurationAVG( arlTwoLane.increasedShoulderDuration ),
							avgIncidentDurationWithSavings: _this.incidentDurationAVG( segment.get( strPeak + 'IncreasedShoulderBlockageAVGID') ),
							numManagedIncidents: arlTwoLane.compliantDrivers
						});
					default:
						var arlOneLane = _this.getCompliance( segment, strPeak, 'OneLane', driverRemovalLaw );
						blockages.push({
							name: 'oneLaneBlockage',							
							savings: Number( avgDurationSavings ),
							avgIncidentDuration:  _this.incidentDurationAVG( arlOneLane.laneDuration ),
							...
							numManagedIncidents: arlOneLane.nonCompliantDrivers
						});
						blockages.push({
							name: 'improvedOneLaneBlockage',							
							savings: 0,
							avgIncidentDuration:  _this.incidentDurationAVG( arlOneLane.improvedLaneDuration ),
							avgIncidentDurationWithSavings: _this.incidentDurationAVG( segment.get( strPeak + 'ImprovedOneLaneBlockageAVGID') ),
							numManagedIncidents: arlOneLane.compliantDrivers
						});
						blockages.push({
						...
						blockages.push({
							name: 'increasedShoulderBlockage',
							laneSaved: 'One Lane Blockage',		
							savings: 0,
							avgIncidentDuration: _this.incidentDurationAVG( arlOneLane.increasedShoulderDuration ),
							avgIncidentDurationWithSavings: _this.incidentDurationAVG( segment.get( strPeak + 'IncreasedShoulderBlockageAVGID') ),
							numManagedIncidents: arlOneLane.compliantDrivers
						});
					break;
		...
		// @param boolean driverRemovalLaw - if true, we will use the improved blockages.
		this.calculateDelaySavings = function( segment, projectState, driverRemovalLaw ) {
			driverRemovalLaw = driverRemovalLaw || false;
			var _this = this;
			// build peak object
			...
			_.each(peaks, function( peak ) {
				// Build peak object
				tmpPeak = Object.create( _this.buildPeak( segment, peak, driverRemovalLaw ) );
      ...
      var fuelCost = _this.fuelPrices[projectState];
			_.each( objPeaks, function( peak ) {
				...
				_.each( peak.incidentsByWeather, function( meta ) {
					...
					if( meta.blockage == "increasedShoulderBlockage" ) {
						meta.secondaryIncidentsWithoutTIM = 0;
					} else {
						meta.secondaryIncidentsWithoutTIM = segment.get('secondaryIncidentsPercentage') / 100 * meta.incidents;
					}
				});

			});

			_this.calculateTotalPeakSavings( objPeaks, driverRemovalLaw );

			return objPeaks;

		};

		this.calculateTotalPeakSavings = function( arrPeaks, driverRemovalLaw ) {
			var _this = this;
			...
			_.each( arrPeaks, function( peak ) {
				...
				_.each( peak.incidentsByWeather, function( meta ) {
					...
					if( meta.blockage !== 'increasedShoulderBlockage' ) {
						...
						totalSecondaryIncidents += meta.secondaryIncidentsWithoutTIM;
					} else {
						totalPeakTravelDelaySavings -= meta.travelDelaySavings;
						totalPeakTravelDelayCarSavings -= meta.travelDelayCarSavings;
						totalPeakTravelDelayTruckSavings -= meta.travelDelayTruckSavings;
		...
		this.calculateDriverRemoval = function( segment ) {
			...
			var complianceBefore = segment.get('complianceBefore') / 100;
			complianceBefore = ( isFinite(complianceBefore) ) ? complianceBefore : 0;
			var complianceAfter = segment.get('complianceAfter') / 100;
			complianceAfter = ( isFinite(complianceAfter) ) ? complianceAfter : 0;
			var shortenedDuration = segment.get('shortenedDuration');
			...
			function calculateCompliance( peak ) {
				...
				var complianceRate = complianceAfter - complianceBefore;
				// Calculate the number of compliant drivers.
				var compliantDrivers = proportionedIncidents * complianceRate;
				...
				segment.set( peak + '_ShortenedBlockageAVGID', shortenedDuration);
			} 
